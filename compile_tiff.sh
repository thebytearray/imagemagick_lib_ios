...
tiff () {
    ...
    if [ "$1" == "arm64-sim" ]; then
        ...
        try ./configure --prefix="${TIFF_LIB_DIR}_${BUILDINGFOR}" --enable-shared --enable-static --disable-cxx --disable-zstd --host=aarch64-apple-darwin
        tiff_compile
        restore
        ...
        armflags $1
        echo "[|- CONFIG $BUILDINGFOR]"
        try ./configure --prefix="${TIFF_LIB_DIR}_${BUILDINGFOR}" --enable-shared --enable-static --disable-cxx --disable-zstd --host=arm-apple-darwin
        tiff_compile
        restore
        ...
        intelflags $1
        echo "[|- CONFIG $BUILDINGFOR]"
        try ./configure --prefix="${TIFF_LIB_DIR}_${BUILDINGFOR}" --enable-shared --enable-static --disable-cxx --disable-zstd --host="${BUILDINGFOR}-apple-darwin"
        tiff_compile
        restore
        ...
        macx86flags
        echo "[|- CONFIG $BUILDINGFOR]"
        try ./configure --prefix="${TIFF_LIB_DIR}_${BUILDINGFOR}" --enable-shared --enable-static --disable-cxx --disable-zstd --host="${MAC_HOST_TRIPLE}"
        tiff_compile
        restore
    else
